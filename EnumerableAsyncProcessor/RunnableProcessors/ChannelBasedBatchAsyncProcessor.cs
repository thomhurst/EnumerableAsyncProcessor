...
/// <summary>
/// Channel-based batch processor for action tasks, providing producer-consumer pattern with backpressure support.
/// </summary>
public class ChannelBasedBatchAsyncProcessor : AbstractAsyncProcessor
{
    private readonly ChannelProcessorOptions _options;

    internal ChannelBasedBatchAsyncProcessor(
        int count, 
    ...
    internal override async Task Process()
    {
        ...
        Channel<ActionTaskWrapper> channel;
        if (_options.Capacity.HasValue)
        {
            channel = Channel.CreateBounded<ActionTaskWrapper>(_options.CreateBoundedChannelOptions());
        }
        else
        {
            channel = Channel.CreateUnbounded<ActionTaskWrapper>(_options.CreateUnboundedChannelOptions());
        }
...
/// <summary>
/// Channel-based batch processor for item action tasks, providing producer-consumer pattern with backpressure support.
/// </summary>
public class ChannelBasedBatchAsyncProcessor<TInput> : AbstractAsyncProcessor<TInput>
{
    private readonly ChannelProcessorOptions _options;
    ...
    {
        // Create the appropriate channel type
        Channel<ItemTaskWrapper<TInput>> channel;
        if (_options.Capacity.HasValue)
        {
            channel = Channel.CreateBounded<ItemTaskWrapper<TInput>>(_options.CreateBoundedChannelOptions());
        }
        else
        {
            channel = Channel.CreateUnbounded<ItemTaskWrapper<TInput>>(_options.CreateUnboundedChannelOptions());
        }
