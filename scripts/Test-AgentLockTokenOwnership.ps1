...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) "eap-agent-lock-owner-tests-$suffix"
...
function Invoke-AgentLocks {
    ...
    $previousOwnerId = [Environment]::GetEnvironmentVariable('EAP_AGENT_LOCK_OWNER_ID', 'Process')
    $previousThreadId = [Environment]::GetEnvironmentVariable('CODEX_THREAD_ID', 'Process')

    try {
        [Environment]::SetEnvironmentVariable('EAP_AGENT_LOCK_OWNER_ID', $EnvironmentOwnerId, 'Process')
        ...
        [Environment]::SetEnvironmentVariable('EAP_AGENT_LOCK_OWNER_ID', $previousOwnerId, 'Process')
...
function Expire-TestLock([string]$LockName) {
    $deleted = & docker exec $redisContainer redis-cli DEL "eap:agent-lock:$LockName" "eap:agent-lock-meta:$LockName"
...
function Assert-OwnerIdentityNotExposed([string]$LockName, [string]$OwnerId) {
    $meta = & docker exec $redisContainer redis-cli GET "eap:agent-lock-meta:$LockName"
...
function Remove-TestLocks([string[]]$LockNames) {
    $keys = @($LockNames | ForEach-Object { "eap:agent-lock:$_"; "eap:agent-lock-meta:$_" })
