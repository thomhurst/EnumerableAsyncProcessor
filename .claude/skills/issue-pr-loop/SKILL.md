...
$worktreeRoot = Join-Path (Split-Path $repo -Parent) "EnumerableAsyncProcessor-worktrees"
...
If the sandbox cannot write to the sibling directory, use `C:\tmp\EnumerableAsyncProcessor-worktrees` instead.
...
This repo has no Aspire AppHost and its tests need no external services. Do not run `aspire start`, `aspire stop`, or AppHost cleanup commands in this repo.

The only Docker container this loop uses is the `eap-agent-locks-redis` lock container managed by `scripts/AgentLocks.ps1`. Never blanket-delete Docker containers or volumes; another agent may be using them.
...
This repo tracks decomposition with GitHub **native sub-issues** (parent/child) and **dependencies** (`blocked_by` / `blocking`) when available, not markdown checklists as source of truth. Prefer native `blocked_by` relationships for blockage. Repo is `thomhurst/EnumerableAsyncProcessor`. Write calls need the numeric issue `.id` (an integer distinct from `#number`, passed with `-F`, not `-f`).
...
```powershell
gh api repos/thomhurst/EnumerableAsyncProcessor/issues/<N>/sub_issues --paginate --jq '.[] | "\(.number) [\(.state)] \(.title)"'
gh api repos/thomhurst/EnumerableAsyncProcessor/issues/<N>/dependencies/blocked_by --jq '[.[].number]'
```
...
3. Link the child to the parent as a native sub-issue:
   ...
   ```powershell
   gh api --method POST repos/thomhurst/EnumerableAsyncProcessor/issues/<N>/sub_issues -F sub_issue_id=$childId
   ```

4. For ordering, mark a child blocked by its prerequisite (use the prerequisite's `.id`):

   ```powershell
   gh api --method POST repos/thomhurst/EnumerableAsyncProcessor/issues/<blockedNum>/dependencies/blocked_by -F issue_id=<blockingId>
   ```
...
Search for the API, type, processor, builder step, or scheduling behavior the issue references. If it does not exist, the issue is asking you to build it; that is the true scope.

Use `rg` across source and tests before estimating scope:

```powershell
rg "<symbol|type|option>" EnumerableAsyncProcessor EnumerableAsyncProcessor.Pipeline EnumerableAsyncProcessor.UnitTests EnumerableAsyncProcessor.Example README.md -g "*.cs" -g "*.md"
```
...
4. Run focused tests, then the full suite. Tests use TUnit: build `EnumerableAsyncProcessor.UnitTests`, run the produced test executable (or `dotnet run --project EnumerableAsyncProcessor.UnitTests -c Release --framework net9.0`), and use `--treenode-filter` for focus. The library multi-targets — run the full suite on every target framework (`dotnet test`) before opening the PR. For TUnit focused runs, use one `--treenode-filter` pattern per command unless using valid TUnit OR parentheses.
...
- Library code (`EnumerableAsyncProcessor`, `EnumerableAsyncProcessor.Pipeline`) must use `ConfigureAwait(false)` on awaited tasks; tests do not need it.
- The library multi-targets several frameworks — keep changes compatible with the lowest TFM in the `.csproj`; verify with a full `dotnet build` of the solution, not just the newest target.
- This is a concurrency library: changes to processors, throttling, or scheduling need tests covering cancellation, exception propagation, and completion ordering — not just the happy path.
- Flaky tests indicate real bugs, especially here where tests exercise parallelism. Do not rerun CI just to get green; investigate timing, synchronization, and shared-state causes.
